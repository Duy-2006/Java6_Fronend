...
interface Promotion { id?: number; name?: string; discountValue?: number|string; startDate?: string; endDate?: string; applyType?: string }

interface Props { promotion?: Promotion; books: Book[]; categories: Category[]; selectedBookIds?: number[]; selectedCategoryIds?: number[] }

export default function PromotionForm({ promotion, books, categories, selectedBookIds = [], selectedCategoryIds = [] }: Props) {
  const isEdit = !!promotion?.id;
  const router = useRouter();

  const [form, setForm] = useState({
    name: promotion?.name ?? "", discountValue: promotion?.discountValue ?? "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEdit
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/promotions/${promotion!.id}`
        : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/promotions`;
      const res = await fetch(url, {
        method: isEdit ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form,
          bookIds:     form.applyType === "BOOK"     ? [...selBooks] : [],
          categoryIds: form.applyType === "CATEGORY" ? [...selCats]  : [],
        }),
      });
      if (res.ok) { router.push("/admin/promotions"); router.refresh(); }
  ...
  return (
    <>
      <style>{`
        ...
        .today-note{font-size:11px;color:#9ca3af;margin-top:4px}
      `}</style>
          ...
          <div className="promo-card">
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="pf-group">
                <label className="pf-label">Tên khuyến mãi <span style={{color:"#c0392b"}}>*</span></label>
                <input className={`pf-input ${errors.name ? "error" : ""}`} type="text"
                  value={form.name} onChange={e => set("name", e.target.value)}
                  placeholder="VD: Sale hè 2026" maxLength={100} />
                <FieldError msg={errors.name} />
                <div style={{textAlign:"right",fontSize:11,color:"#9ca3af",marginTop:2}}>{form.name.length}/100</div>
              </div>

              <div className="pf-group">
                <label className="pf-label">Giá trị giảm (%) <span style={{color:"#c0392b"}}>*</span></label>
                <input className={`pf-input ${errors.discountValue ? "error" : ""}`} type="number"
                  value={form.discountValue} onChange={e => set("discountValue", e.target.value)}
                  placeholder="VD: 10" min={1} max={100} />
                <FieldError msg={errors.discountValue} />
              </div>

              <div className="pf-date-row">
                <div className="pf-group">
                  <label className="pf-label">Ngày bắt đầu <span style={{color:"#c0392b"}}>*</span></label>
                  <input className={`pf-input ${errors.startDate ? "error" : ""}`} type="date"
                    value={form.startDate} onChange={e => set("startDate", e.target.value)} />
                  <FieldError msg={errors.startDate} />
                </div>
                <div className="pf-group">
                  <label className="pf-label">Ngày kết thúc <span style={{color:"#c0392b"}}>*</span></label>
                  <input className={`pf-input ${errors.endDate ? "error" : ""}`} type="date"
              ...
              <div className="pf-group">
                ...
                <select className="pf-input" value={form.applyType} onChange={e => set("applyType", e.target.value)}>
              ...
              <p className="pf-note">Nếu không chọn sách hoặc thể loại → hệ thống hiểu là áp dụng toàn bộ</p>
            </form>
          </div>
        </div>
          ...
          <div className="selection-panel">
            {form.applyType === "ALL" && (
              <div className="empty-state">
                ...
                <p>Chọn <strong style={{color:"#6366f1"}}>Theo sách</strong> hoặc <strong style={{color:"#6366f1"}}>Theo thể loại</strong><br/>để hiển thị danh sách tại đây</p>
              </div>
            )}
            ...
            {form.applyType === "BOOK" && (
              <div>
                ...
                <input className="search-box" type="text" placeholder="🔍 Tìm kiếm sách..."
                ...
                <div className="select-bar">
                  ...
                  <button type="button" className="select-all-btn" onClick={() => {
                    if (allBooksSelected) { setSelBooks(new Set()); } else { setSelBooks(new Set(filteredBooks.map(b => b.id))); }
                    setAllBooksSelected(!allBooksSelected);
                  }}>{allBooksSelected ? "Bỏ chọn tất cả" : "Chọn tất cả"}</button>
                </div>
                <div className="item-grid">
                  {filteredBooks.map(b => (
                    <div key={b.id} className={`item-card ${selBooks.has(b.id) ? "selected" : ""}`}
                      onClick={() => { setSelBooks(toggle(selBooks, b.id)); setErrors(v => ({ ...v, applyType: undefined })); }}>
            ...
            {form.applyType === "CATEGORY" && (
              <div>
                ...
                <input className="search-box" type="text" placeholder="🔍 Tìm kiếm thể loại..."
                  value={catSearch} onChange={e => setCatSearch(e.target.value)} />
                <div className="select-bar">
                  <span className="count">{selCats.size} đã chọn</span>
                  <button type="button" className="select-all-btn" onClick={() => {
                    if (allCatsSelected) { setSelCats(new Set()); } else { setSelCats(new Set(filteredCats.map(c => c.id))); }
                    setAllCatsSelected(!allCatsSelected);
                  }}>{allCatsSelected ? "Bỏ chọn tất cả" : "Chọn tất cả"}</button>
                </div>
                <div className="item-grid">
                  {filteredCats.map(c => (
                    <div key={c.id} className={`item-card ${selCats.has(c.id) ? "selected" : ""}`}
                      onClick={() => { setSelCats(toggle(selCats, c.id)); setErrors(v => ({ ...v, applyType: undefined })); }}>
