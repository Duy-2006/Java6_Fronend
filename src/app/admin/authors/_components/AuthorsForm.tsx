...
export default function AuthorForm({ author }: { author?: Author }) {
  ...
  const [form, setForm]     = useState({ id: author?.id ?? null, name: author?.name ?? "", email: author?.email ?? "" });
  const [errors, setErrors] = useState<ReturnType<typeof validateAuthor>>({});
  const [loading, setLoading] = useState(false);

  const set = (field: string, value: string) => {
    setForm(f => ({ ...f, [field]: value }));
    // Clear lỗi ngay khi user sửa
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEdit
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/authors/${form.id}`
      ...
      if (res.ok) {
        router.push(`/admin/authors?success=${encodeURIComponent(isEdit ? "Cập nhật thành công." : "Thêm tác giả thành công.")}`);
        router.refresh();
      } else {
          ...
          <div className="card border-0 shadow-lg mt-4">
            <div className="card-header text-white py-3" style={{ background: "linear-gradient(135deg, var(--primary), var(--primary-dark))", borderRadius: "0.5rem 0.5rem 0 0" }}>
              <h5 className="m-0 fw-bold text-uppercase d-flex align-items-center">
                {isEdit ? <i className="fa-solid fa-pen-nib me-2" /> : <i className="fa-solid fa-user-plus me-2" />}
              ...
              <form onSubmit={handleSubmit} noValidate>
                ...
                <div className="mb-4">
                  ...
                  <div className="input-group">
                    <span className="input-group-text bg-light"><i className="fa-solid fa-signature text-muted" /></span>
                    <input
                      type="text"
                      className={`form-control form-control-lg ${errors.name ? "is-invalid border-danger" : ""}`}
                      value={form.name}
                      onChange={e => set("name", e.target.value)}
                      onBlur={() => setErrors(v => ({ ...v, ...validateAuthor({ name: form.name }) }))}
                ...
                {/* Email */}
                <div className="mb-4">
                  <label className="form-label fw-bold text-secondary">Email Liên Hệ (Tùy chọn)</label>
                  <div className="input-group">
                    <span className="input-group-text bg-light"><i className="fa-solid fa-envelope text-muted" /></span>
                    <input
                      type="email"
                      className={`form-control form-control-lg ${errors.email ? "border-danger" : ""}`}
                      value={form.email}
                      onChange={e => set("email", e.target.value)}
                      onBlur={() => { if (form.email) setErrors(v => ({ ...v, ...validateAuthor({ name: form.name, email: form.email }) })); }}
                      placeholder="contact@author.com"
                    />
                  </div>
                  <FieldError msg={errors.email} />
                  <div className="form-text text-muted small mt-1 ms-1">Email dùng để liên hệ bản quyền hoặc hợp tác.</div>
                </div>

                <div className="d-flex gap-2 justify-content-end mt-5 pt-3 border-top">
                  <a href="/admin/authors" className="btn btn-light border fw-bold px-4">
                  ...
                  <button type="submit" disabled={loading} className="btn btn-primary fw-bold px-4 shadow-sm">
                    ...
                    {loading ? "Đang lưu..." : isEdit ? "Cập nhật" : "Lưu hồ sơ"}
                  </button>
                </div>
              </form>
            </div>
