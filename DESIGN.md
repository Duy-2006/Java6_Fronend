...
### Brand & Accent
- **Red Primary** ({colors.primary}): The buy-now CTA color (vibrant red `#C92127`). Used on every "Add to cart", "Configure", "Pre-order" button inside the commerce flow and the right-rail purchase panel.
- **Deep Red** ({colors.primary-deep}): Pressed-state and dark-surface variant of the primary red (`#A8171C`); also the active link color.
- **Soft Red** ({colors.primary-soft}): Translucent background tint for informational callouts (`rgba(199, 33, 39, 0.08)`).
